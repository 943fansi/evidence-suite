...
- **默认要求**：所有事实性/争议性论断必须带 `[Sx]`；来源须经过审核方 r1 审计。
- 未经审计的 w2 检索内容 → 标注 `[待内部确认]`，不得用作支撑性证据。
- 每个核心章节默认至少 **2 个独立来源** 交叉支撑关键论断（可调，但只能更高不能更低）。
- **降低严谨层级**（如允许单来源）仅当用户明确同意且风险可控；**提高严谨层级**用于方法论、核心贡献、结论等关键段落。

### 参考文献数量下限（Minimum Source Count）
...
1. 先读题目 → 建立 Topic Card（不可跳过，锁定后续所有阶段锚点）。
