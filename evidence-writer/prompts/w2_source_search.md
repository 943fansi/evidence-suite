...
1. **覆盖达标**：每个关键问题（P1–P3）与每项关键技术（T1–T3）都有 ≥1 条对应来源，且核心论断有 ≥2 条独立来源交叉支撑。
2. **反证已查**：每个关键主张都已执行反证主动搜索（criticism/limitation/contradictory/failed），并记录结果或负结果。
3. **边际收益衰减**：连续新增 ≥5 条来源，但未覆盖任何新关键主张、也未产生新的 contradictory evidence。
4. **多样性达标**：来源覆盖 ≥2 个独立国家/机构，论文类非单一作者/课题组/期刊。

满足 1+2+4 即停止；若此时来源数仍 < {min_sources}，在 `evidence_gaps` 如实记录"来源数未达下限但边际收益已衰减"，交由审核方 r1 判断退回补搜或接受——**不得灌无关来源凑数**。
...
8. 来源数量下限：返回的 sources 数量不得少于 {min_sources}（开题报告/可行性报告≥15，调研报告/综述≥25，本科≥20，硕士≥40，博士≥60，白皮书≥12，GF报告≥12，实施方案≥12，期刊论文≥15，专利交底书/申请草案≥8）。若检索结果不足，扩大数据库、年限、语言或相邻领域关键词后继续检索，不得以低质量/无关资料凑数。（触发「证据停止规则」后，即使未达下限也如实记录缺口，不再为凑数扩检。）
