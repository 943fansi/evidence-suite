...
20. 为每条来源标注 `freshness`（current / recent / historical / superseded / unknown，见 `${SUITE_ROOT}/shared/references/claim_evidence_layer.md` 的 Evidence Freshness）：按来源日期与现行状态判定；标准类 `superseded` 须剔除或替换，政策/法规须 `current`。缺标注视为缺失字段。
