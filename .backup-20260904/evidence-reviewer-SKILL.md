...
description: |
  证据驱动红队审查方 — 与 evidence-writer 形成对抗循环，负责审查门流水线（r1 来源审计 → r2 诚实性自评 → r3 框架深度门 → r4 初稿审查 → r5 外部专家评审 → 终审门）。有罪推定、只找失败、不写正文、严重即阻断。高置信触发：证据审计、来源审计、source audit、draft review（仅限带 [Sx] 引用的事实性文档）、evidence-driven 审查、source-grounded 审查、红队审查。运行需 Python 3.10+、Bash 与联网能力（仅 live 审计模式联网）。NOT_TRIGGER：普通语法纠错、纯润色、拼写检查、非事实性文案点评、通用文档 review（不带 [Sx] 引用的）；显式禁用标记 --evidence-suite-disable 也将跳过整个审查流水线。
version: "0.2.1"
compatibility: "evidence-suite ≥ 0.2.0（与 evidence-writer 同版本号同步）"
allowed_tools:
  - Read
  - Write
  - Edit
  ...
  - WebSearch
  - WebFetch
disallowed_tools: []  # live r1 模式允许 WebSearch/WebFetch；其余默认 static
---
